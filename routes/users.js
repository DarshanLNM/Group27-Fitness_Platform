...
router.get('/account', async (req, res) => 
{
	...
	try
	{
		...
		res.render('users/useraccount', {username: loggedUser.username, nickname: loggedUser.nickname, 'post-list':posts, loggedUser});
	}
	catch(e)
	{
...
router.post('/signin', async (req, res) => 
{
    ...
    res.status(401).render('home/signin', {message: "Error : Credentials do not match. No Account? Register now!"}); 
});
...
{ 
	...
	try
	{
    	...
    	else
    	{
        	...
        	if((password[0].match("[a-zA-Z0-9]" === null) || (password[0].match(regex) === null)))
        	{
...
{ 
	...
	try 
	{
		...
		res.render('users/useraccount',{username: updatedUser.username, nickname: updatedUser.nickname, 'post-list':posts, success1:success1, success2:success2, success3:success3, loggedUser: updatedUser}); 
	} 
	catch (e) 
	{ 
		let message1;
		...
		{
			message3 = "Error : Password must contain a capital letter, a numerical value and a special character";
		}
		...
		res.status(404).render('users/useraccount',{username: ous, nickname: odn, 'post-list':posts, message1:message1, message2:message2, message3:message3, message4:message4, loggedUser: oldUser}); 
	}
});
